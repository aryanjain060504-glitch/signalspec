...
\> Replace \`\<YOUR\_DOMAIN\_UI\_LIB>\` with whatever fits your industry (e.g. FullCalendar for scheduling, Chart.js for analytics, react-map-gl for mapping, etc.)
...
\| **\*\*Domain UI\*\***        | \`\<YOUR\_DOMAIN\_UI\_LIB>\`        | —                | Industry-specific          |
...
   ├── PROJECT\_BLUEPRINT.md           # This file

   └── BACKEND\_PLANNING.md            # Detailed planning doc
...
\- Never commit \`.env\`, \`node\_modules\`, \`dist/\`, or \`build/\`
...
**\*\*Never commit:\*\*** real secrets, \`node\_modules\`, or generated bundles.
...
\- **\*\*\`node\_modules/\`\*\*** — must be ignored everywhere it exists (root, \`backend/\`, \`frontend/\`). Lockfile (\`package-lock.json\` or \`pnpm-lock.yaml\`) **\*\*is\*\*** committed for reproducible installs.
...
\- **\*\*Verify before first push:\*\*** \`git status\` must not list \`.env\` or \`node\_modules\`. Use \`git check-ignore -v path\` if unsure.
...
MONGODB\_URI=mongodb://localhost:27017/your\_db\_name

\# Production: mongodb+srv://\<user>:\<pass>@cluster.mongodb.net/your\_db
...
JWT\_ACCESS\_SECRET=replace\_with\_64\_char\_hex

JWT\_REFRESH\_SECRET=replace\_with\_different\_64\_char\_hex
...
GOOGLE\_CALLBACK\_URL=http\://localhost:5000/api/v1/auth/google/callback
...
ENCRYPTION\_KEY=replace\_with\_64\_char\_hex



\# ── Email (Nodemailer) ────────────────────────────────────────────────────

SMTP\_HOST=smtp.resend.com

SMTP\_PORT=465

SMTP\_USER=resend

SMTP\_PASS=your\_smtp\_api\_key

EMAIL\_FROM=noreply\@yourdomain.com
...
VAPID\_EMAIL=mailto\:admin\@yourdomain.com



\# ── Monitoring ────────────────────────────────────────────────────────────

SENTRY\_DSN=https\://your\_sentry\_dsn\_here
...
VITE\_VAPID\_PUBLIC\_KEY=your\_vapid\_public\_key
...
 MONGODB\_URI: z.string().url(),

 JWT\_ACCESS\_SECRET: z.string().min(32),

 JWT\_REFRESH\_SECRET: z.string().min(32),

 JWT\_ACCESS\_EXPIRES\_IN: z.string().default('15m'),

 JWT\_REFRESH\_EXPIRES\_IN: z.string().default('7d'),

 CLIENT\_URL: z.string().url(),

 CORS\_ORIGINS: z.string(),

 ENCRYPTION\_KEY: z.string().length(64),

 // Optional — only required if feature is enabled

 GOOGLE\_CLIENT\_ID: z.string().optional(),

 GOOGLE\_CLIENT\_SECRET: z.string().optional(),

 SMTP\_HOST: z.string().optional(),
...
 origin: env.CORS\_ORIGINS.split(','),
...
if (env.NODE\_ENV === 'development') app.use(morgan('dev'));



// 7. Global rate limit

const globalLimiter = rateLimit({ windowMs: 60\_000, max: 100, standardHeaders: true, legacyHeaders: false });
...
const authLimiter = rateLimit({ windowMs: 60\_000, max: 10, skipSuccessfulRequests: true });
...
   "code": "ERROR\_CODE",           // Machine-readable constant

   "message": "Human readable",    // Display to user

   "fields": {                     // Only present on VALIDATION\_ERROR
...
 { "success": false, "error": { "code": "TOKEN\_EXPIRED", "message": "Access token expired" } }
...
 { "success": false, "error": { "code": "NOT\_FOUND", "message": "Resource not found" } }
...
\| 400 | \`VALIDATION\_ERROR\` | Zod validation failed on body/query/params |

\| 400 | \`INVALID\_REQUEST\` | Logically invalid request (e.g. end before start) |

\| 400 | \`CONFIRM\_TEXT\_MISMATCH\` | Destructive action confirmation text wrong |

\| 401 | \`UNAUTHORIZED\` | No token provided |

\| 401 | \`TOKEN\_EXPIRED\` | Access token expired — client should refresh |

\| 401 | \`TOKEN\_INVALID\` | Token tampered or wrong secret |

\| 401 | \`REFRESH\_TOKEN\_INVALID\` | Refresh token not found or already rotated |

\| 403 | \`FORBIDDEN\` | Authenticated but wrong role |

\| 403 | \`CANNOT\_SELF\_DEMOTE\` | User tried to remove their own admin/owner role |

\| 404 | \`NOT\_FOUND\` | Resource not found or doesn't belong to this user |

\| 409 | \`CONFLICT\` | Duplicate resource (unique field already exists) |

\| 429 | \`RATE\_LIMIT\_EXCEEDED\` | Too many requests in window |

\| 500 | \`INTERNAL\_ERROR\` | Unhandled server error — check Sentry |
...
\- \`JWT\_ACCESS\_SECRET\` and \`JWT\_REFRESH\_SECRET\` are different, each ≥64 chars

\- \`ENCRYPTION\_KEY\` is 32 random bytes (64 hex chars) — stored in env only
...
 baseURL: import.meta.env.VITE\_API\_BASE\_URL + '/api/v1',
...
       error.response?.data?.error?.code === 'TOKEN\_EXPIRED' &&
...
 ALLOWED\_ATTR: ['href', 'target']

});

\<div dangerouslySetInnerHTML={{ \_\_html: safeHTML }} />
...
\- \`VITE\_\*\` env vars validated with Zod at build time — no runtime crashes
...
Industry: \<YOUR\_INDUSTRY>

Primary resources: \<RESOURCE\_1>, \<RESOURCE\_2>, \<RESOURCE\_3>
...
\- Validate VITE\_API\_BASE\_URL and all VITE\_\* vars with Zod in src/lib/env.ts
...
\- Domain-specific UI: \<YOUR\_DOMAIN\_UI\_LIBRARY>
...
2\. Root .gitignore (and package roots if split) — node\_modules/, .env\*, keys, dist/build; only .env.example for secrets template; lockfile committed

3\. .cursorignore at repo root mirroring sensitive paths (.env\*, node\_modules, dist, keys) per Section 9
...
7\. Full API documentation for each endpoint following the template in PROJECT\_BLUEPRINT.md
...
Resources: \<RESOURCE\_1>, \<RESOURCE\_2>
...
21\. .gitignore verified — .env\*, node\_modules, dist, keys never tracked; only .env.example committed
...
12\. VITE\_\* env vars validated with Zod at build time
...
Resources: events (4 types: event/task/out\_of\_office/appointment), calendars, reminders, teams

Special features:

\- RRule (RFC 5545) for recurrence; rrule.js on both server and client

\- Event edit modes: "this" / "this\_and\_following" / "all"

\- date-fns-tz for UTC ↔ user timezone conversion; all dates stored as UTC

\- Team availability: free/busy query with calendarVisibility (full/busy\_only/none)
...
Resources: dashboards, widgets, data\_sources, reports
